...
class Administration extends Database
{
    ...
    public function  adminLogin(){
        ...
        if(isset($_SESSION['connecter']) && $_SESSION['connecter'] === true){
            header('Location: http://localhost/gite/admin/administration');
        }else{
            header('Location: http://localhost/gite/admin/login');
        }
        ...
        if(!empty($_POST['email_admin']) && !empty($_POST['password_admin'])){
            ...
            if($stmt->rowCount() >= 1){
                ...
                if($_POST['email_admin'] == $row['email_admin'] && $_POST['password_admin'] == $row['password_admin']){
                    ...
                    //La redirection
                    header('Location: http://localhost/gite/admin/ndex.php?url=administration');
                }else{
    ...
    public function  userLogin(){
        ...
        if(isset($_SESSION['connecter_user']) && $_SESSION['connecter_user'] === true){
            ...
            echo "<p class='alert-warning mt-2 p-2'>Vous n'ètes âs encore inscrit sur notre site
                    <a href='signup' class='btn btn-info'>S'incrire</a>
                </p>";
        }
        ...
        if(!empty($_POST['email_user']) && !empty($_POST['password_user'])){
            ...
            if($stmt->rowCount() >= 1){
                //CReer une variavle qui liste (recherche) tous les element
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                $id = $row['id_user']; //id de la table users de phpmyadmin
                //Recup de email
    ...
    public function registerUser(){
        ...
        if($insert_user){
            header("Location: http://localhost/gite/admin/accueil");
        }else{
