...
class Gites extends Database
{
                ...
                <div class="col-4 mt-3">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <a href="index.php?url=details&id=<?= $row['id_gite'] ?>" class="btn btn-outline-info">Détails</a>
                            <br /><br />
                            <a href="index.php?url=update&id=<?= $row['id_gite'] ?>" class="btn btn-outline-warning">Editer</a>
                            <br /><br />
                            <a href="index.php?url=delete&id=<?= $row['id_gite'] ?>" class="btn btn-outline-danger">Suprimer</a>
                        </div>
                    </div>
                </div>
                <?php
    ...
    public function getGiteById($id_gite){
        $db = $this->getPDO();
        $req = $db->prepare("SELECT * FROM gites INNER JOIN category_gites ON gites.gite_category = category_gites.id_category WHERE gites.id_gite = ?");
        $id = $_GET['id'];
        $req->bindParam(1, $id);
        $req->execute();
        $res = $req->fetch();
        ...
        <div>
            ...
            <div class="row mt-5">
                <div class="col-6">
                    ...
                    <a href="reservation&id=<?= $res['id_gite'] ?>" class="btn btn-outline-info mt-3">RESERVER</a>
                    <br>
                    <?php
    ...
    public function addGite(){
        ...
        try {
            ...
            if ($insert) {
                header("Location: http://localhost/gite/admin/index.php?url=administration");
            } else {
    ...
    //Supprimer un gite
    public function giteToDelete($id_gite){
                ...
                <div id="gite-dispo" class="col-sm-12 col-lg-4 mt-3">
                    <div id="gite-card" class="card">
                        ...
                        <div class="card-body">
                            ...
                            <a href="index.php?url=details&id=<?= $row['id_gite'] ?>" class="btn btn-outline-info mt-2">Plus d'infos</a>
                        </div>
                    </div>
                </div>
                <?php
                    ...
                    <div class="col-4 mt-3">
                        <div class="card" style="width: 22rem;">
                            ...
                            <div class="card-body">
                                ...
                                <p><b>Date de depart : </b></p>
                                <p class="alert-info p-2"> <?=  $date_d->format('d-m-Y')?></p>
                                <a href="index.php?url=details&id=<?= $row['id'] ?>" class="btn btn-outline-info">Plus d'infos</a>
                            </div>
                        </div>
                    </div>
    ...
    public function disabledGite(){
        ...
        $sql = "UPDATE gites SET disponible = 0 WHERE id_gite = ?";
        //Requète préparée
        ...
        if($res){
            ...
            header("Refresh: 2, http://localhost/gite/admin/accueil");
        }else{
            //Sinon on affiche une erreur
            ...
            <div class="col-4 mt-3">
                <div class="card" style="width: 22rem;">
                    ...
                    <div class="card-body">
                        ...
                        <p><b>Date de depart : </b></p>
                        <p class="alert-info p-2"> <?=  $date_d->format('d-m-Y')?></p>
                        <a href="details.php&id=<?= $row['id_gite'] ?>" class="btn btn-outline-info">Plus d'infos</a>
                    </div>
                </div>
            </div>
            <?php
