...
class Reservation
{
    public function reserverGite(){
        ...
        try {
            ...
            $mail->Username = '8a3e0252469823'; //Generer lors de la création du compte mailTrap = dans l'espace mailtrap roue crantée + smtp setting -> zendFramework https://mailtrap.io/inboxes/1163067/messages
            $mail->Password = 'cf7dceddc82ca0'; // Idem pour le mot de passe
            ...
            $user = "root";
            $pass = "";
            $db = new PDO("mysql:host=localhost;dbname=gite;charset=utf8;", $user, $pass);
            $query = "SELECT * FROM gites INNER JOIN category_gites ON gites.gite_category = category_gites.id_category WHERE gites.id_gite = ?";
            ...
            while ($datas = $req->fetch()) {
                ...
                $emailId = $datas['id_gite'];
                //Url du liens de validation
                $url = "http://localhost/gite/admin/confirmer_reservation&id=$emailId";
                //Contenus du mail
                $mail->Body = '
    <!DOCTYPE html>
    <html>
